...
class Xcloner_Api
{
    ...
    public function save_schedule()
    {
        ...
        if ($id !== null) {
            ...
            $schedule['status'] = 1;
            $schedule['start_at'] = strtotime($this->form_params['backup_params']['schedule_start_date'] . " " . $this->form_params['backup_params']['schedule_start_time']);
        ...
        if (!$schedule['start_at']) {
            ...
            $schedule['start_at'] = date('Y-m-d H:i:s', $schedule['start_at'] - $this->xcloner_settings->get_xcloner_option('gmt_offset') * \HOUR_IN_SECONDS);
        }
        ...
        if ($id === null) {
            $this->xcloner_database->insert($this->xcloner_database->prefix . 'xcloner_scheduler', $schedule, array('%s', '%s'));
        } else {
            $this->xcloner_database->update($this->xcloner_database->prefix . 'xcloner_scheduler', $schedule, array('id' => $id), array('%s', '%s'));
        }
        if ($id !== null) {
    ...
    public function backup_files()
    {
        ...
        $data['finished'] = 1;
        try {
            $data = $this->archive_system->start_incremental_backup($this->form_params['backup_params'], $this->form_params['extra'], $init);
        } catch (Exception $e) {
            $data = array();
            $data['error'] = \true;
            $data['status'] = 500;
        ...
        if ($data['finished']) {
            if (isset($this->form_params['backup_params']['email_notification']) && $to = $this->form_params['backup_params']['email_notification']) {
                try {
                    ...
                    $this->archive_system->send_notification($to, $from, $subject, $data['extra']['backup_parent'], $this->form_params, "", $additional);
                } catch (Exception $e) {
    ...
    private function process_params()
    {
        ...
        if ($params && isset($params->processed)) {
            $this->form_params = json_decode(json_encode((array) $params), \true);
            return;
        }
    ...
    public function get_file_system_action()
    {
        ...
        if ($folder == "#") {
            ...
            $data[] = array(
                ...
                //'children' => true,
                'state' => array('selected' => \false, 'opened' => \true),
    ...
    public function get_database_tables_action()
    {
        ...
        if ($database == "#") {
            ...
            foreach ($return as $database) {
                ...
                $data[] = array('id' => $database['name'], 'parent' => '#', 'text' => $database['name'] . " (" . (int) $database['num_tables'] . ")", 'children' => \true, 'state' => $state, 'icon' => plugin_dir_url(dirname(__FILE__)) . "/admin/assets/database-icon.png");
            }
        } else {
            try {
            ...
            foreach ($return as $table) {
                $state = array();
                if ($xcloner_backup_only_wp_tables && !stristr($table['name'], $this->xcloner_database->prefix)) {
                    continue;
                }
                if (isset($database['name']) && $database['name'] == $this->xcloner_database->dbname) {
                    $state = array('selected' => \true);
                }
                $data[] = array('id' => $database . "." . $table['name'], 'parent' => $database, 'text' => $table['name'] . " (" . (int) $table['records'] . ")", 'children' => \false, 'state' => $state, 'icon' => plugin_dir_url(dirname(__FILE__)) . "/admin/assets/table-icon.png");
            }
        }
        $this->send_response($data, 0);
    }
    ...
    public function get_schedule_by_id()
    {
        ...
        $data['start_at'] = date("Y-m-d H:i", strtotime($data['start_at']) + $this->xcloner_settings->get_xcloner_option('gmt_offset') * \HOUR_IN_SECONDS);
    ...
    public function get_scheduler_list()
    {
        ...
        foreach ($data as $res) {
            ...
            if (trim($next_run)) {
                $date_text = date($this->xcloner_settings->get_xcloner_option('date_format') . " " . $this->xcloner_settings->get_xcloner_option('time_format'), $next_run_time + $this->xcloner_settings->get_xcloner_option('gmt_offset') * \HOUR_IN_SECONDS);
                if ($next_run_time >= time()) {
            ...
            if ($res->last_backup) {
                if ($this->xcloner_file_system->get_storage_filesystem()->has($res->last_backup)) {
                    ...
                    $backup_time = date($this->xcloner_settings->get_xcloner_option('date_format') . " " . $this->xcloner_settings->get_xcloner_option('time_format'), $metadata['timestamp'] + $this->xcloner_settings->get_xcloner_option('gmt_offset') * \HOUR_IN_SECONDS);
                }
    ...
    public function backup_encryption()
    {
        ...
        try {
            ...
            $this->logger->info($iv);
            $return = array_merge($return, $this->xcloner_encryption->encrypt_file($backup_file, "", "", $start, base64_decode($iv)));
        } catch (Exception $e) {
            $return['error'] = \true;
    ...
    public function backup_decryption()
    {
        ...
        $return['total_size'] = filesize($this->xcloner_settings->get_xcloner_store_path() . DS . $backup_file);
        try {
            $return = array_merge($return, $this->xcloner_encryption->decrypt_file($backup_file, "", $decryption_key, $start, base64_decode($iv)));
        } catch (Exception $e) {
            $return['error'] = \true;
            $return['message'] = $e->getMessage();
        }
    ...
    public function get_manage_backups_list()
    {
        ...
        foreach ($backup_list as $file_info) {
            ...
            $file_exists_on_local_storage = \true;
            ...
            if (!isset($file_info['parent'])) {
                ...
                if (!$file_exists_on_local_storage) {
                    ...
                    <a href="#"
                       title="<?php 
                    echo __("This backup archive does not exist on your local server. If you wish to retain local copies of your backup archives as well as remote copies, please disable local clean-up in the main XCloner settings and/or your scheduled backup profile settings.", "xcloner-backup-and-restore");
                    ?>">
                ...
                if (isset($file_info['childs']) && is_array($file_info['childs'])) {
                    ...
                    foreach ($file_info['childs'] as $child) {
                        ?>
                            <li>
                                <?php 
                        echo esc_html($child[0]);
                        ?> (<?php 
                        echo esc_html(size_format($child[2]));
                        ?>)
                                <?php 
                        $child_exists_on_local_storage = \true;
                        if ($storage_selection) {
                        ...
                        if (!$storage_selection) {
                            ...
                            if ($this->xcloner_encryption->is_encrypted_file($child[0])) {
                                ?>
                                        <a href="#<?php 
                                echo esc_attr($child[0]);
                                ?>" class="backup-decryption"
                                           title="<?php 
                                echo __('Backup Decryption', 'xcloner-backup-and-restore');
                                ?>">
                                ...
                                ?>
                                        <a href="#<?php 
                                echo esc_attr($child[0]);
                                ?>" class="list-backup-content"
                                           title="<?php 
                                echo __('List Backup Content', 'xcloner-backup-and-restore');
                                ?>"><i
                                                    class="material-icons">folder_open</i></a>

                                        <a href="#<?php 
                                echo esc_attr($child[0]);
                                ?>" class="backup-encryption"
                                           title="<?php 
                                echo __('Backup Encryption', 'xcloner-backup-and-restore');
                                ?>">
                            ...
                            ?>
                                    <a href="#<?php 
                            echo esc_attr($child[0]);
                            ?>" class="copy-remote-to-local"
                                       title="<?php 
                            echo __('Push Backup To Local Storage', 'xcloner-backup-and-restore');
                            ?>"><i
                ...
                ?>
                <?php 
                echo esc_html(size_format($file_info['size']));
                ?>
                <?php 
                $return['data'][$i][] = ob_get_contents();
                ob_end_clean();
                ...
                <a href="#<?php 
                echo esc_attr($file_info['basename']);
                ?>" class="download"
                   title="<?php 
                echo __('Download Backup', 'xcloner-backup-and-restore');
                ?>">
                    <i class="material-icons">file_download</i>
                </a>
                <?php 
                if (!$storage_selection) {
                    ...
                    if (sizeof($available_storages)) {
                        ...
                        ?>" class="cloud-upload"
                           title="<?php 
                        echo __('Send Backup To Remote Storage', 'xcloner-backup-and-restore');
                        ?>">
                ...
                ?>" class="delete"
                   title="<?php 
                echo __('Delete Backup', 'xcloner-backup-and-restore');
                ?>">
                ...
                if ($storage_selection && !$file_exists_on_local_storage) {
                    ...
                    ?>" class="copy-remote-to-local"
                       title="<?php 
                    echo __('Transfer a copy of the remote backup to local storage.', 'xcloner-backup-and-restore');
                    ?>">
                        <i class="material-icons">swap_horiz</i>
                    </a>
                <?php 
    ...
    public function list_backup_files()
    {
        ...
        if (isset($data['extracted_files']) && is_array($data['extracted_files'])) {
            foreach ($data['extracted_files'] as $file) {
                ...
                $return['files'][$i]['mtime'] = date($this->xcloner_settings->get_xcloner_option('date_format') . " " . $this->xcloner_settings->get_xcloner_option('time_format'), $file->getMtime());
                $i++;
            }
        }
        if (isset($data['start'])) {
            ...
            $return['finished'] = 0;
        } else if ($this->xcloner_file_system->is_multipart($source_backup_file)) {
            $return['start'] = 0;
            ++$return['part'];
            if ($return['part'] < sizeof($backup_parts)) {
                $return['finished'] = 0;
    ...
    public function upload_backup_to_remote()
    {
        $this->xcloner_container->check_access();
        $return = array();
        if (isset($_POST['data']) && $data = json_decode(stripslashes($_POST['data']), \true)) {
        ...
        try {
            if (method_exists($xcloner_remote_storage, "upload_backup_to_storage")) {
                $return = call_user_func_array(array($xcloner_remote_storage, "upload_backup_to_storage"), array($backup_file, $storage_type, $delete_local_copy_after_transfer));
            }
        } catch (Exception $e) {
            $return['error'] = 1;
            $return['message'] = $e->getMessage();
